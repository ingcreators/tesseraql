...
— an official runtime image, a Helm chart, a stop that drains and ends inside its grace,
readiness that consults the members, `tesseraql bench` and a capacity guide, alerts that page
once per cluster — and the defects the proof's rehearsals found on the way are fixed here. The
defaults moved to where production runs: the front door admits what a member admits,
`maxInFlight` stands on its own at 40, jobs and file transfers can leave the online pool for
pools of their own, and the stack's framework pool is sized by declaration. A connection now
says whose it is on PostgreSQL, SQL Server, Oracle, MySQL and MariaDB, and a vanished
PostgreSQL host fails a statement in about a minute instead of never. A new application carries
production and staging profiles that separate its pools, take the JWT secret from the
environment, and turn metrics and the retention sweep on; it declares its time and language,
and a named profile refuses the published development secret. A source can be held and every
committing writer declares what it made stale, which completes Phase 32; a list view exports
its filtered set, and My exports lists what a user started. The framework moves to Jackson
3.1, the long-term-support line, and reads YAML as YAML 1.2; Vert.x is 5.2.0. The procurement
demo is finished: it prints its documents and exchanges its receipt notice over SFTP with a
second gallery member. **Includes pre-1.0 breaking changes** — YAML's core schema
(`yes`/`no`/`on`/`off` are text in a boolean field, `0123` is decimal) and content after a
JSON value or YAML document refused; `tools.jackson` types on the MCP, security and YAML
modules' public methods, and Jackson 2 gone from the runtime; `TQL-SEC-4154` under a named
profile; `maxInFlight` no longer derived from `workerThreads`, and the front door's share
raised to 40; `deploy/Dockerfile` derives from the runtime image; the list page's markup
contract (`commandfor` on the Filters opener, the export kick-off adding its card, the chrome
its swaps refresh, the export controls); the task queue moved to its own bundled app — each
...
- **A new application carries a production profile that separates its pools.** `tesseraql new`
  ...
  base configuration, which `tesseraql dev` runs, keeps one pool. An application with these files
...
- **The procurement demo prints its quotation.** `GET /api/supplier/quotes/{id}/print`
  ...
  same template, and `quotes_scope` on both sources so a competitor's quote is a row
  outside the caller's reach. The app declares the codec module (`tesseraql.modules`) and
  its locales (`tesseraql.i18n`, Japanese first). `ProcurementDocumentsIntegrationTest`
  reads the rendered text back — the title, the supplier's name, the total — which is
  the proof the embedded font carries the glyphs. Record:
  `docs/procurement-documents-and-edi.md` (S1).

- **The sample font covers JIS X 0208 level 1.** `TesseraQLSampleGothic-Regular.ttf`
  held 422 glyphs — the kanji the examples happened to render — and lacked 47 of the 434
  code points the procurement demo prints, 見 and 積 among them; the codec drew each as
  a box and said nothing. The font is now the level-1 subset of Noto Sans JP (ASCII, rows
  1–8, the level-1 kanji, halfwidth katakana: 4,749 glyphs, 1.35 MB), regenerated by
  `scripts/sample-font.py`, in `examples/user-admin-app/fonts/`,
  `examples/procurement-app/fonts/` and the pdf and runtime modules' test resources.

- **A document with no row answers a status, not a blank page.** `export.statusWhen` on a
  `query-export`: the first arm whose condition is truthy over the route's sources
  (`header.rowCount == 0`) answers its status — judged before the extraction opens, so a
  template never dereferences an empty header, and again with `main.rowCount` once the rows
  are written. The answer is the error envelope with `TQL-LD-2863` and the declared status.
  A `file-export` and a job's export step refuse the key at lint and boot (`TQL-YAML-1041`).
  Found by the procurement quotation: a competitor's quote id, a row outside the caller's
  reach, was a 500 (`TQL-LD-2831`) from the template's own dereference.
...
- **A transfer records what its export's follow-up announces.** `tql_file_transfer` gains
  `emit_json`, `invalidates_json` and `emit_tenant_id` (`V16__transfer_announcement.sql`, three
  vendors, applied by the bootstrap too), recorded at start. `ExportRequest` carries `emit`,
...
- **Hypermedia Components 0.4.2, and the four briefs it answers.** Two drop-in bumps, 0.4.1
  then 0.4.2 (each diffed WebJAR to WebJAR: nothing removed; `.hc-shell__group`, one token, six
  behaviors and two catalog keys added; the email artifacts differ by their version stamp
  alone), answer the four briefs TesseraQL filed from its 0.4.0 review, so four stand-ins go.
  The shell's sidebar captions are the kit's `.hc-shell__group` and the collapsed rail centers
  its own items, so `tesseraql.css` loses its caption rule and both rail rules (brief 13). The
  Studio flags page's switches carry the kit's `data-hc-submit-on-change` and the bootstrap's
  `change` listener is gone (brief 15). The dialog opener is the platform's: upstream answered
  brief 14 with HTML's invoker commands rather than a kit attribute, so **the route compiler
  now emits `commandfor="<view>-filters" command="show-modal"` on every list page's Filters
  button and applied-filter chip where it emitted `data-tql-open-dialog="#<view>-filters"`**,
  and the Studio shell's command-palette trigger follows — a markup-contract change. The
  command is Baseline 2025 (Chrome and Edge 135, Firefox 144, Safari 26), and the Filters
  dialog is the only way to compose a filter, so the kit carries the fallback (brief 16):
  `installInvokerCommands` performs `show-modal` and `close` only where `HTMLButtonElement` has
  no `commandForElement`, and retires itself where the platform answers, so an iPad that stays
  on Safari 18 still opens the dialog. The kit states its browser floor once on a *Browser
  support* page (the Popover API; a functional loss above it gets a scripted fallback, a
  cosmetic one none), and `docs/hypermedia-ui.md` gains "Opening a dialog" and "Browser
  support", which inherits that floor. The manifest guard pins `installSubmitOnChange` and
  `installInvokerCommands`. The recipe sweeps — the repository, not the WebJAR — found no
  recipe added; the one that changed, chat-messages, grew two textarea attributes the
  copilot's single-line composer has no use for.
...
- **Under `--embedded-db`, `main` keeps the pool size it declares.** `main` was built from the
  ...
  HikariCP's defaults. Its role pools already read their sizing under the same override. Now the
  override supplies only the coordinate. `docs/cli-surface.md` decision 4b had promised this;
  its query parameters were carried over, and now the sizing is too.
  `docs/capacity-defaults.md` decision 11.
...
- **A download-timed export's first fetch works under shared-schema tenancy.** The transfer
  ...
  per-tenant pool — through a resolver that shared-schema never binds — so the fetch read its
  own tenant as unknown and answered `500` (`TQL-LD-2810`, "no per-tenant datasource resolves
  it") for every tenant since 0.18.0. The per-tenant pools are wired to the transfer service
  only in a per-tenant mode; without them the follow-up runs on the shared pool, where the
  request that started the export ran. Found by the download-timed row of
  `ResultHoldIntegrationTest`, the first to fetch such an export under `shared-schema`.
...
- **A print template's `#{key}` reads the application's catalogs.** The PDF codec's template
  ...
  locale. The export now carries the application's `messages/` catalogs over the framework's
  built-ins (`FileWriteSpec.messages`, `DocumentMessages`), resolved once per document in the
  locale it renders in; a key no catalog holds still shows the marker. `PdfFileCodecTest`
  renders both, and the procurement quotation's labels are the proof over HTTP.
...
- **A keep-alive client is shed during the drain.** The stack's front kept serving a pooled
  ...
  then the close once the streams in flight are done. The client reconnects through the
  Service to a pod that stays, and the in-flight count reaches zero within a round trip. The
  front then closes only once it has been quiet for a whole linger: the count covers what the
  relay accepted, not a client's next request already on the wire, and a close at the first
  zero cut exactly
...
- **A request racing a hot reload no longer answers 404, and never crashes on a torn chain.**
  ...
  registration replaces the pipeline in place, so the removal bought nothing. And the builder's
  step list was filled on the watcher thread while request threads copied it —
  `ConcurrentModificationException` on the request thread, a 500 for a caller whose save it
  wasn't. The reloader now compiles changed routes in place (removing only on a failed compile,
  where the 500 stub takes over, and for routes that are actually gone), and the builder is
  synchronized so every racing lookup sees a consistent snapshot. What a mid-compile lookup
  sees is unchanged and deliberate: the chain as it stands, never cached (the registry's
  version check), never torn.
