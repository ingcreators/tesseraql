...
/**
 * The {@link io.tesseraql.pipeline.Exchange} property names the framework's steps agree on.
 *
 ...
 * parameter expressions. {@link #SQL_PARAMS} holds the resolved bind values for the SQL step.
 *
 * <p>Properties are not headers: they travel with the exchange and never reach a client, so they
 * carry no filtered namespace. The one exception below is {@link #ACTING_ROLE_HEADER}, which is a
 * wire header and is named like one.
 */
public final class TesseraqlProperties {
     ...
     *
     * <p>An exchange property, not a message header — which is why it takes the {@code Tesseraql}
     * prefix of its neighbours here rather than the {@code tql.} namespace {@link
     * io.tesseraql.pipeline.Headers} uses: properties never reach the wire, so no filter reads
     * this name.
     */
    public static final String EXCEPTION_CAUGHT = "TesseraqlExceptionCaught";
    ...
    /**
     * The transport this runtime serves on, published by the HTTP server once it knows
     * (docs/vertx-native.md decision 6): the host's shared instance or the one it built. A
     * record to read, not a channel to configure through — the server takes its transport as a
