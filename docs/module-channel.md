...
2. ~~**The Windows app-image classpath route.**~~ **Settled in slice 4b: `app\ext\` plus one
   `app.classpath=$APPDIR\ext\<jar>` line in the generated `.cfg`.** The jpackage job asserts it in
   both directions on `windows-latest` — a stack whose framework datasource is SQL Server is
   refused with `TQL-APP-4220` before the jar is placed, and gets past that refusal after. An image
   upgrade replaces the app directory, so the placement is repeated on upgrade; that is the same
   contract the container image's `lib/` has.
