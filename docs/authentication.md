...
| `TQL-FIELD-2002` | 400 | **The request body must be a JSON object** — a body these endpoints could not read, at any of them: sign-in, sign-out, the session endpoints, token exchange, or the recovery legs. It covers a truncated body, valid JSON that is not an object (`[]`, `123`, `"abc"`), and the literal `null`. The sentence arrives in `details.message`; the envelope's own `message` is the status phrase. |
