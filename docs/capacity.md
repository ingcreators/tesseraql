...
- **Connections are a standing number, not a peak.** A pool holds `maximumPoolSize` connections
  ...
  pools, against that tenant's database. Under a stack, add the stack's own sign-in pool: when
  `tesseraql-stack.yml` declares `framework.datasource`, the stack framework pool (its
  `maximumPoolSize`, 10 by default), which the stack surface also uses as its `main`; otherwise
  the surface's own `main`, 10 connections. Multiply by the nodes, and keep the total under the
  database's ceiling.
