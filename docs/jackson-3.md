...
> then it is deleted (decision 6). **Shipped, #1449**, as designed, with the findings of row 19:
> a fourth trap in the library (a builder-made YAML factory loses `EMPTY_STRING_AS_NULL`), node
> accessors that throw where 2.x answered `""` or `0` (main code keeps 2.x's answers), eight
> ad-hoc mappers where row 13 had found one, and an example whose outbox id was always `null`.
...
| 19 | Measured in S1 (the slice's own findings) | **a. A fourth trap, in the library.** `YAMLFactory.builder().build()` starts its YAML read features at zero in 3.1, so `EMPTY_STRING_AS_NULL` is off; `new YAMLFactory()` has it on (write features are unaffected). `v:` read as `""` instead of null until `YamlMappers` enabled it by name, and the ledger now pins it. **b. Node accessors.** 3.x's `asString()` throws on an object or array (2.x's `asText()` answered `""`) and answers `""` for JSON null (2.x: `"null"`); `asInt()`, `asLong()`, `asBoolean()` and `asDouble()` throw on a value they cannot coerce (2.x: `0`/`false`); `asString(default)` applies its default to containers too. On a malformed client payload that is a 500 where there was a 400, so main code's 55 `asString()` became `asString("")`, and the SCIM `active` and Copilot `index` reads take defaults; test code keeps the throwing accessors. **c. A null read as text.** Three user-admin example routes bound `eventId: main.eventId`, a name nothing carries: the field was always `null`, and the two tests asserting it passed because 2.x read the null as `"null"`. **d. Test sources.** 188 ad-hoc `new JsonMapper()` constructions ran on 3.x defaults (18 view-renderer tests red on an absent boolean) and moved onto the factories. **e. The recipe also** deleted `YamlMappers`' comments and one `import java.io.IOException`, and wrote explicit `3.1.7` versions into ten module poms under the BOM (removed). snakeyaml-engine's `CoreScalarResolver` takes `supportMerge` (`false`: Jackson reads no merge keys). **f.** Until S3, Jackson 2's `jackson-core` resolves at vertx-core's own 2.21.6, no longer managed by a BOM. **g. The generated YAML surface reference** changed one cell, `map of ` to `map of any`: an `additionalProperties` whose `type` is a list, where 2.x's `asText("any")` answered `""`. **h. Bench** (a 200-row JSON route, 8 workers, 30 s after a 10 s warm-up, PostgreSQL in a container, the same machine): before 10,813.7 req/s, p50/p95/p99 0.7/1.0/1.2 ms; after 10,790.6 req/s, 0.7/1.0/1.3 ms. |
...
- **A SCIM PATCH's `active: "True"` reads false**, in Jackson 2 and 3 alike: a string boolean is
  recognised only as lowercase `"true"`, and Azure AD sends `"True"`/`"False"`. Found reading the
  accessors in S1 (row 19b); not a migration change. *Trigger: an Azure AD provisioning report,
  or the next SCIM interoperability pass.*
...
| `JsonCreatorModeTest` (new): a scan of every main source for a `@JsonCreator` without `mode =` | `LockSpec.of` with the mode removed |
