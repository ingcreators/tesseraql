...
- **XD-07j's fix shape.** The declared-header source for the 109 shell routes was designed by
  ...
  proven harmful by two shipped ITs; the shape that keeps them green is not yet proven.
...
|---|---|---|---|---|
| 1 | `job run` migrates before it bootstraps | XH-01, unfiled 28-29 | S | `JobCommand.wire` → `FrameworkMigrations.migrateOperations(main)` before the first `ensureSchema()`; H2/DuckDB skip inside `migrateComponent`; correct the Javadoc; CHANGELOG names the hand repair (drop the operations tables AND `tql_schema_history__operations`; CLI-run rows are lost). Guard: `JobCommandIntegrationTest` — `job run` on a fresh database, then a runtime boot against it; red on HEAD (V3 42701), red on a variant deleting the call. Never edit the scripts. |
| 2 | The workflow engine resolves for the document it is told about | G32, G33, G34, G35, G36, G37 | L (2 PRs) | 2a the transactional side: `assign:` told its document (`/* key */`), reminders keep `recipient:` and tenant, both linted (G32, G34). 2b the sweeper: a scope resolver on `escalate` (G33), one failing breach does not roll back the others and the WARNING names the task (G35), `reassign` binds declared `params:` and a no-row answer is loud (G36), delegation rules honoured under tenancy (G37). Three procurement gallery resolvers are affected — the gallery ITs are the guard. |
| 3 | Tenancy reaches every executor, and refuses what it cannot route | G24, G25, G27, G28, G29, G30, G31 | L (2 PRs) | 3a file-export/import on the tenant's pool and 4031 on an unknown tenant (G24; the slice-6 deferral's "no caller" premise is false for route-triggered transfers), an unknown `tenancy.mode`/`resolver.type` refused at boot (G25). 3b scope arms sharing a bind name (G27), a non-main export source's own `params:` (G28), TQL-TENANT-3001 over `steps:` and non-main sources again (G29 — lost in #753), `/*%scope*/` in a non-main export source (G30), the transfer subtree scoped by tenant (G31, medium — EH-01's product decision, now with a security row behind it). |
| 4 | An account's second factor and its invite survive what the operator did | G38, G39, G42, G40, G43, G41, G26 | M | `totp/begin` on a confirmed enrollment re-checks the password (G38); an invite token dies with Disable and the console can withdraw it (G39); confirm activates the recovery codes in the same transaction and the SQL Server V2 `rowversion` is measured (G42); mTLS `clockSkew` widens the window (G40); the last `tql.iam.admin.write` holder cannot disable themselves (G43); the audit-trail sentence names the Java-mounted routes it does not cover (G41); the account
card and two pages stop saying an elevation "expires by itself" (G26). |
| 5 | A `between` cell means what it says | G8 | S | `DecisionTables.java:417`: `> n`/`< n` compile to an inclusive bound shifted by the literal's scale. Every decision table with a strict bound is wrong at the boundary; the gallery's are the guard. |
| 6 | The Studio data browser reads one schema and writes one row | DN-06f, unfiled 2, DN-06g, F108, unfiled 39-40 | S | `connection.getSchema()` as `schemaPattern` and in `TableRef`; `updateRow` transactional (rollback on `affected != 1`); the note folded; a log line at both catches; `row` refuses several matches. `uiDataBrowserExportOfAnUnreadableTableIsANote` is rewritten, not deleted. Guard: the two-schema fixture with the default `_pkey` name — red on HEAD by a three-row overwrite. |
| 7 | The host writes the catalogue; a refusal is loud and does not loop | XD-08a, XD-08b, unfiled 16-18, 57-61 | M | Addendum to `runtime-replace.md` structural decision 2 first ("the CLI writes candidates, the host writes the catalogue and the status"); then `AppUpgrader`'s deploy/promote/rollback write a candidate, the host moves the catalogue on an applied replace, a refused candidate is recorded once with action and version and not re-attempted, `--url`/console/`rollback` report the host's verdict (`rollback --wait`), `deploy status` never renders a stale refusal, the three published sentences corrected. Guards: `StackReconcilerTest` (refused → catalogue unmoved, one attempt), a plain-filesystem `MultiAppHost` boot with a refusing catalogued member (stack up on the last served), `AppUpgraderTest` rollback → the last served version. |
| 8 | The compiler refuses what lint should have, with a route's name on it | unfiled 1, 4, 5, XD-07c, XD-07f, XH-22, XD-07g, unfiled 6, 35, 37 | M | Every lint-green boot NPE becomes a coded refusal naming the route (a missing `response:`, a step with neither `file:` nor `contract:`, a view with no `recipe:`, a file-import without `import:`/a row step/a file), plus lint rows for each; `conditions.zone` and a bad cron shaped (the cron lint needs a dependency decision — sequence with F60); the lint returns one finding for an unparseable document and `--format json` always prints a document (the extension then publishes it). Guard hazard: the view NPE escapes before the runtime's wrapper — the guard is on `ViewSpec.parse`. |
| 9 | An input is fed by what the route declares; a response is framed | XD-07j, XD-07l, unfiled 67-68, DN-02a, DN-02c, XD-09b, unfiled 20, 47, 50, 13 | M (2 PRs) | 9a: a declared header source (`header.Cookie`) adopted by the 109 shell routes, then `rawValue:348` returns null, the precedence made one mechanism (`Request.param`), `body.` on a GET and an undeclared `body.<name>` linted — recorded as a breaking change; the shell ITs and a `RequestBinderHeaderTest` are the bracket. 9b: a header NAME refused where E3 refused the value (lint, boot, edge); a `Content-Disposition` under `headers:` with a placeholder linted (DN-02c); a spooled download declares its length so an HTTP/1.0 client — nginx at its default — sees a truncated body as truncated and the gateway has nothing to buffer (XD-09b, unfiled 20; the `hosting.md` ingress paragraph with it); the root redirect encodes its query; `Cache-Control` precedence; `<source>.first` documented as what it is. |
| 10 | The dev MCP server validates `Origin` | G1, G2, G6, G7, G5, G4, G3, unfiled 48 | M | The spec's MUST on a loopback server: `Origin` validated, `Content-Type` required, a body bound, the session required after `initialize`; the double-coded message (G2); the resource reachable under the stack issuer (G6); the four flat refusals shaped (G7); the Apps UI page says what the fragment does (G5); the input schema carries `pattern`/`minLength`/formats (G4); G3's two surviving legs (batches accepted, `MCP-Protocol-Version` judged) without narrowing `SUPPORTED`; the challenge and default resource spelled for a non-ASCII app name (unfiled 48). |
| 11 | The expression language fails as coded, and once | G11, G12, G13, G14, G15, F121, unfiled 11, G9, G10 | M | A relational null is a coded 400 (G11); a map key named `size`/`length`/`empty` wins over the virtual property (G12); the lexer keeps a backslash it does not recognise (G13); numeric compare is exact past 2^53 (G14, medium); `getterValue` stops at getters (G15 — the Javadoc's promise); a literal `matches()` operand compiles once at parse time, a bad one is a lint/boot finding, a non-literal is never cached (F121, unfiled 11); a 2-way SQL file that does not parse and the three unlinted expression positions get their lint (G9, G10). |
| 12 | The suite runner says what it proved | G16, G17, G18, G19, G20, G21, G22, G23, F115, unfiled 27, 10 | M | The coverage gate counts every file (G16); a `sql` case cannot `commit;` (G17); `--fail-on-regression` refuses a corrupt baseline and exits 3 on a regression (G18, F115 — cli-surface decision 10b); a duplicate case name is refused (G19); the four kinds compile against the app's registry (G20); the recorder types query values (G21); the two thresholds gate or are struck from the docs (G22); `RouteTestRunner`/`RouteSuite` are wired or deleted (G23); `test` and `schema` on a refused database answer the operator message at 1 (unfiled 27, 10). |
