...
| chunk `onError: {skipLimit}` (object) vs import/http-source `onError:` (string enum) | chunk becomes `onError: fail \| skip` + sibling `skipLimit:` (default 100 when skipping — bounded, so a wholly broken feed still fails) | a key whose *type* depends on where it sits cannot be frozen |
