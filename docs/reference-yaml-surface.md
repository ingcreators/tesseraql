...
| `emit` | any | Topic(s) broadcast to live views after this route's write commits (docs/realtime.md): a command-json command at its commit, a file-import when its background import's transaction commits, a file-export when its after: statement commits - with the extraction, or on the first fetch of the file for a download-timed one; an export with no after: has no write to announce. A name is lowercase dot/dash-separated segments; the event carries the topic name only, never data. |
| `invalidates` | any | Source table(s) this route's write makes stale: the code catalogs and the held results (a source's `cache:`) that read them are dropped after the commit, on this node at once and on every other within the stamp interval, so a rollback invalidates nothing. Legal on the recipes that commit - command-json, webhook, queue-consume, file-import, where the drop happens when the import's transaction commits, after the response, and file-export, where it happens when the after: statement commits (with the extraction, or on the first fetch of a download-timed one); an export with no after: has no commit to follow. Named by table, not by catalog: a maintenance screen for a shared code master writes a row whose kind is request data, so which catalog is affected is not known until the write happens. Documented in code-catalogs.md and response-shaping.md. |
...
| `groupBy` | string | A column the rows are read as ordered groups by, each exposed to a workbook report template (jxls) as a `key` and its own `rows`. The rows must be ordered by it (TQL-LD-2851). A print (pdf) template reads no groups: group in the query, or write one document per group with `splitBy:` (TQL-YAML-1005). |
...
| `statusWhen` | [statusWhen](#statuswhen) | Conditional statuses a `query-export` answers instead of a document: the first entry whose expression is truthy over the route's sources (`header.rowCount == 0`) decides the status, judged before the extraction opens — so a template never renders a header with no row — and again with `main.rowCount` once the rows are written. The answer is the error envelope with TQL-LD-2863 and the declared status. A `file-export` answers 202 before its rows are read and a job step answers no request, so both refuse the key (TQL-YAML-1041). |
...
| `invalidates` | any | Table(s) this job's steps write that a code catalog or a held source (a source's `cache:`) reads: the catalogs and the held results reading them are dropped once the run ends, provided at least one step committed - a job is not one transaction - and, on a poll-triggered file-import job, when each import's transaction commits. Named by table, as a command's invalidates: is; a table nothing reads lints as TQL-FIELD-4620. Documented in jobs.md and code-catalogs.md. |
...
A hold over fetched rows (docs/caching.md): how long they serve before the source is asked again, and which tables a writer's `invalidates:` drops them by. One shape for a source's statement and for an enrichment's reference.

| Property | Type | Description |
| --- | --- | --- |
| `maxAge` \* | string | How long the rows are held before the source is asked again: a positive duration such as `30s` or `5m`. Required — nothing is held unless the declaration says for how long. |
| `tables` | any | The tables the statement reads, as a writer would name them — what a command's `invalidates:` names to drop this hold on every node. Required on a statement (a hold no write can reach is the defect the declaration exists to prevent); refused on an `http:` reference, which nothing stamps. A string or a list. |
...
| `cache` | [hold](#hold) | Hold each key's rows across requests (docs/caching.md): a partner looked up once serves every later request for `maxAge`, keyed by the reference, the tenant and the key, whatever the surrounding key set. A `sql:` reference names the `tables` a command's `invalidates:` drops it by; an `http:` reference takes `maxAge` alone (nothing stamps a partner system); a `source:` reference fetches nothing and holds nothing. Within one request every block already shares one memo, hold or no hold. Refused where nothing can be held (TQL-YAML-1077). |
...
One acquisition or one statement. Exactly one mechanism arm names the means — sql (a colocated 2-way SQL file), contract (a named identity contract), service (a runtime provider), http (an outbound call) — or the write-side sequence, and that arm nests the keys the mechanism owns. Beside the arms sit the questions no mechanism answers: the when: guard, the enrich: folded into the rows, the result: kinds the rows are read in, and the spool: another step filled. Documented in unified-sources.md.
