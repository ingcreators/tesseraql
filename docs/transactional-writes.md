...
Domains, rules, decisions, scope fragments, calendars and messages are all shared documents
referenced by name — a step sequence was not. The audit note, the counter refresh, the "write the
interface row" tail that a dozen commands repeat was copied into each `steps:` block, and the
copies drift.

There is deliberately no `include:`/`extends:` in the document model, and that stays: a route a
reviewer reads must be whole. This is the `rules/` shape instead — a named artifact with a typed
contract, referenced where it is used:
...
fragments:
  audit-note:
    ...
    steps:
      - id: note
        sql:
          file: audit-note.sql          # colocated with this document
          mode: update
          params:
            entity: binds.entity
...
- **Expansion happens at manifest load.** The fragment's steps become real steps of the
  referencing document, so the transaction, coverage, spans and lints see ordinary steps — no
  runtime indirection, and nothing downstream can tell a fragment was involved.
- **Step ids are prefixed with the reference's id**, joined by `_`: the example's step is
  `audit_note`, addressed as `steps.audit_note.affectedRows`. The gap analysis floated a dotted
  `audit.note`; that is not addressable, because the context resolver splits a path on `.` and
  would look for a step named `audit` containing one named `note`. A collision with a declared
  step id fails the load rather than shadowing it.
- **`binds:` is checked against the reference's `params:` exactly**, both ways: a missing bind
  and an undeclared one both fail the load, the way a shared rule's contract does. A step inside
  the fragment reads one as `binds.<name>`.
- **SQL files are colocated with the fragment document**, and the expansion re-paths them
  relative to the referencing document, so the compiler resolves them exactly as it resolves a
  colocated file.
- **One level.** A fragment cannot `use:` another — the delegation precedent, one hop and never a
  chain, so a reader of the expansion sees the whole of it.
- Command-side first (`steps:`). A job's `pipeline:` can follow if jobs turn out to repeat
  sequences the same way.

The reference is an ordinary step: it carries its own `id:` and the wiring lives inside the arm,
where every other mechanism's wiring lives. The gap analysis sketched `use:`/`as:` as step-level
keys; nesting them keeps the surface's own grammar instead of making `steps:` the one place where
`params:` sits outside an arm.
