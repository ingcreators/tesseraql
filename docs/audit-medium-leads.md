...
| F128 CSV export has no BOM option | **LIVE (widens)** | `dx` as filed; not low — the procurement demo's shipments export hands a user Japanese CSV data mark-less, and `query-export` has no author-level workaround until F82 slice 2 | PR 4c: `bom:` on the `export:` block, default `false` — SHIPPED #nnnn |
