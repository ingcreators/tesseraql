...
| F126 export `timezone:`/`format:` → 500 | **LIVE (widens)** — the code is `TQL-SQL-2500` (never `TQL-ROUTE-5000`), raised by `SqlStep.export`'s catch-all, which files the codec failure as a SQL failure naming the SQL file, after the extraction ran; "Locale is safe by contrast" is false (a mistyped locale renders `Locale.ROOT` at 200, and on the import arms `de_DE` parses `1234,50` as `123450.00`); the configuration key is not static (a per-request source resolves through it); a `zoneinfo` claim reaches `ZoneId.of` as a string, number, array or nested object; a request header of the input's name is an input; the stack trace exists only where a logging backend does | medium | measured record `export-declarations.md`; PR 5-0 SHIPPED #1305 — the two codec crashes the measurement found inside `ColumnValues.toZoned` (an Excel grid/placement NULL cell → 500; a `java.sql.Time` column → 500 on five of six drivers); PR 5a SHIPPED #1306: every literal export declaration judged at lint and boot with `TQL-YAML-1063`; inert keys warn at boot; `import.locale` judged; `tesseraql job run` judges before wiring; PR 5b SHIPPED #1307: a request-sourced zone or locale answers 400 `TQL-FIELD-2001` before any SQL; the chain is literal → source → config → platform on every arm; a job reads `tesseraql.files.*`; a document-write failure is `TQL-LD-2802` — **F126 CLOSED (5a + 5b)**; PR 5c SHIPPED #1308: job-step `splitBy:` runs; a file-export split bundle is recorded and served as `zip`/`application/zip` under the bundle name |
| F127 bundled apps hard-coded English | **LIVE (reframed)** | medium | class GREW in-window; fixed — the end-user apps in 8b, the operator apps in 8c; F127 CLOSED |
| F82 README front-door drift | **LIVE (reframed)** | medium | partially killed; the residue fixed, slice 10; the framework defect — slice 2 of its own campaign, decision 3 — CLOSED by [`codec-discovery.md`](codec-discovery.md) S1-S4 (2026-09-15): the README's second quick start runs as written from the `-Pdist` archive, and CI proves it |
| F87 extension test never runs | **LIVE (widens)** | medium | fixed, slice 10 |
| F88 `.dockerignore` stale exclusions | **LIVE (reframed)** | medium | fixed, slice 10 |
| F90 `mvn verify` presented as the whole check | **LIVE (reframed)** | medium | fixed, slice 10 |
| F91 extension palette names | **LIVE (widens)** | medium | fixed, slice 10 |
| F96 phantom `--hc-*` tokens | **LIVE (reframed)** | medium | fixed, slice 10 |
| F97 `:target` row rules | **LIVE (reframed)** | medium | fixed, slice 10 |
| F98 hypermedia-ui.md teaches a deleted stand-in | **LIVE (reframed)** | medium | fixed, slice 10 |
| F106 failures logged message-only | **LIVE (reframed)** | medium | half-killed by #1291; the export half by export-hygiene P3; the rest fixed, slice 7 |
| F113 `dev --embedded-db` shutdown race | **LIVE** | medium | fixed #1299 |
| F114 CLI usage errors are stack traces | **LIVE (widens)** | medium | fixed, slice 9 |
| F118 `MessageCatalog.live()` per resolution | **LIVE (widens)** | medium | fixed, slice 6 |
| F119 rate-limiter monitor across JDBC claim | **LIVE (reframed)** | medium | untouched |
| F120 `JdbcSessionStore.touched` unbounded | **LIVE (widens)** | medium | fixed, slice 6 |
...
| F99 Studio preview pins `data-theme="dark"` | **LIVE (reframed)** | low (was medium) | fixed, slice 10 |
| F112 `serve` in CLI help text | **LIVE (narrowed)** | low | headline killed; the Javadoc residue swept, slice 10 |
| F89 development-environment.md wrapper step | **LIVE (reframed)** | low | mechanism killed, page orphaned; the page fixed, slice 10 |
...
1. **The framework's Japanese catalog is unreachable by default** (medium-high, i18n).
   ...
   *app's* `messages/` dir only — never the built-in catalog merged one line earlier. **None of the
   ...
   one-line fix (`catalog.tags()`) is unverified and is a default-behaviour decision. *Decided
   and shipped as slice 8a, #1337 (decision 2 below): the served set is every catalog found, app and
   built-in; measured on a zero-config list page, `Accept-Language: ja` renders `lang="ja"` and
   the framework's `データがありません` where HEAD rendered `lang="en"` and `No rows`.*
...
6. **`tesseraql identity-schema --admin-login … --admin-password-file …`, taught by the product's
   ...
   existence guards cannot see absence. *Fixed in slice 9: the hint carries `--app <dir>`
   (`LoginMethodsTest` pins it), and the command without a database is one line and exit 2.*
...
| 3 | An interrupted `dev --embedded-db` stops the database last | F113 | M | SHIPPED #1299. Three pieces, not two, and the two written here do not work alone: the window the library's own hook covers is *inside* `builder.start()`, which no hoisted hook can reach, so the CLI must also choose the data directory and claim the instance before that call. With only the first two, an interrupt during startup — or a gateway port already in use, with no signal at all — leaves a live PostgreSQL behind. Carries a cost of its own, filed: a `kill -9` during the drain now leaks what it used to have already stopped. **Follow-up 2026-09-14**: the interrupt-during-startup guard failed on CI three times in a day (twice on #1329, once on main at `b73201148`), each with a live server on the first attempt's directory and no "Stopping" line in the interrupted run's log. The claim's stop looked for the `postgres` process *at the instant of the interrupt*, and at "postmaster started as" there is none yet: the process the library names is `pg_ctl` (`-w start`), which forks, `setsid`s and execs a shell that execs the server — measured locally, 22 ms later. The JVM's shutdown hooks do not stop the start's own thread, so the stop now waits for the start to conclude and closes its handle; a stop that lands before the claim refuses the start. Guard: `EmbeddedPostgresOwnershipTest`, a stop placed during `initdb` and one placed before the claim, in-process — both red on `877e46631`, each exactly red on its own variant. |
...
| 5 | An export declaration is refused, or it takes effect | F126 | S+M+M+S | Four PRs plus a record (`export-declarations.md`), not one M: N3 is dead as filed and N2 is deferred with the zoneless-shift decision; nothing is shared with 4c. What the row was missing: an Excel grid/placement NULL cell → 500 and a `java.sql.Time` column → 500 (PR 5-0), `import.locale` corruption, the unshaped boot refusals, the config-level source expression and the missing job-path config fallback (5a/5b), the request-time 4xx before any SQL and the fallback decision (5b), job-step and file-export `splitBy:` (5c). The lint/boot gap for `bom:` on `excel`/`pdf` closes by a boot WARN from the linter's own predicate, never a refusal. COMPLETE (5-0 #1305, 5a #1306, 5b #1307 — F126 closed — 5c #1308). |
| 6 | Bound the accumulators by their unit of work | F120, F118 | S+S | **SHIPPED**, one PR. F120: `touch()` sweeps entries older than `TOUCH_INTERVAL` once per interval (an `AtomicLong` cadence guard, one sweeper at a time), so the map holds the sessions seen in the last minute; every `Instant.now()` in the store reads an injected `Clock` (a sixth constructor argument; the five-argument one passes `Clock.systemUTC()`), and `throttledTouches()` is the diagnostic the guard reads. Guard in `JdbcSessionStoreIntegrationTest` (tesseraql-runtime; the security module has no database test dependency): a hundred sessions touched, thirty seconds on a touch sweeps nothing (101), sixty-one seconds on a touch sweeps the hundred (2) — `v-nosweep` answers 102. F118: not the throttled signature the audit suggested first (a throttle window would break `I18nIntegrationTest.aMessageCatalogEditIsServedLiveWithoutARestart`, which asserts the VERY next render — the edit and the GET are milliseconds apart) but the memo the Javadoc always claimed: `Templates.render` reads `MessageCatalog.live` once per render and publishes it to the Thymeleaf context (`Templates.CATALOG_VARIABLE`, a root-level variable that survives fragment levels), and `CatalogMessageResolver` takes it from there, calling `live()` only for a context without one. `MessageCatalog.directoryListings()` counts the listings; guard `MessageCatalogTest.aRenderListsTheCatalogDirectoryOnce`: twenty keys, one render, one listing, and the edited catalog on the next render at the same cost — `v-nomemo` answers 21. |
| 7 | A failure leaves its throwable | F106 | S | **SHIPPED.** Slice 2 (#1298) had already given `ClusterRateLimiter`'s degrade line its throwable and export-hygiene P3 (#1311) the export arm's; a census of `LOG.warn/error(… getMessage()/toString())` over the main sources on 2026-09-14 found 13 message-only sites. Fixed 10: `JobExecutor:405` (the run-level catch — the step-level one records and rethrows, so one stack, not two), the file-import catch and both import-spool sites in `JdbcFileTransferService`, the three `MultiAppHost` close failures, the topic bridge in `TesseraqlRuntime`, `RouteWatcher`'s new-directory failure, `StackRelay`'s 502 line (which now also names the path it was forwarding — `appName` is declared inside the try), and the outbox's dead-lettering line. Left message-only on purpose: the outbox's retried attempt (it comes back; the terminal one carries the stack), `RouteWatcher:147` (`ex.toString()` on an ignored watch event) and `StackReconciler:247` (a torn catalogue read is expected and transient). Guard: `FailureThrowableIntegrationTest` (tesseraql-runtime — the operations module has no slf4j provider on its test classpath, per slice 2): a `select 1 / 0` job step and an import declared `format: excel` fed CSV bytes (the codec throws before any row, which is the outer catch), stderr captured, the WARN line and `\tat io.tesseraql.operations…` frames under it; both red on origin/main's operations jar (installed with `-Dmaven.test.skip=true`), the WARN line present and no frames. The record's caveat held: the four-placeholder "silent SLF4J trap" variant cannot be built red at SLF4J 2.x. |
| 8 | The framework's own locales are reachable | (unfiled N1), F127 | S + M-L + L | N1 first, or the template swap has no observable effect. **8a SHIPPED #1337** (N1, decision 2): `I18nSettings.from` serves every catalog found — the app's files and the built-in `en`/`ja` — where it served the app's files alone; with it the `defaultLocale: ja_JP` item slice 5 filed here (`normalize` folds it to `und`, every catalog lookup for `und` throws `TQL-YAML-1007`, so the app booted and answered 500 to every error response, and `tesseraql lint` crashed in `I18nRules` on the same lookup) — a declared `defaultLocale`/`locales` entry is judged by 5a's one locale rule (`ExportDeclarations.isFormattableLocale`) at lint (`I18nRules`, one finding per entry at its key) and at boot (`from` refuses the first), both `TQL-YAML-1065`; `CatalogLocaleRules.lintCatalogLanguages` skips when the declarations do not hold, or it would build the settings the boot refuses. Guards: `DefaultLocalesIntegrationTest` (a zero-config list page in `ja`, and the misspelling refusing the boot at its key), `LocaleResolutionTest.theBuiltInJapaneseNegotiatesWithoutAnAppCatalog` (in the module that owns `tesseraql/messages/ja.yml` — the yaml module's own tests cannot see the built-in files), `I18nSettingsTest` and `AppLinterI18nTest` for the judge. Bracket: head + `v-appcatalog` (N1 reverted) + `v-nofrom` (lint only) + `v-nolint` (boot only) + `v-nocatalogskip`, each caught by exactly its own assertion — the first head column was GREEN on every guard: `install -DskipTests` still compiles test sources, the new tests do not compile against HEAD's class, the install failed and the fixed jar stayed in `.m2`; re-run with `-Dmaven.test.skip=true` and the jar proven by its constant pool. The full verify then found the one test that pinned the defect: slice 5's `theNegotiatedLocaleUnderAnUndDefaultIsServedAsToday` booted an app on `ja_JP` to show the `und` request locale served at 200 (`export-declarations.md` decision 27); its premise is gone, it is deleted, and decision 27 carries the note. **8b SHIPPED**: the eleven end-user templates (`account` 4, `auth-ui` 5, `portal` 2) carry no hard-coded text — 189 new `tql.*` keys in the built-in catalog, `en` and `ja` (`BuiltinCatalogParityTest` keeps them in step), the `{0}` MessageFormat placeholders where a value rides in a sentence, `th:utext` where the message carries `<code>`, `th:attr` for the confirm dialog's three attributes, `#{…}` as the shell fragment's title argument; the inbox timestamp is the `Instant` formatted by `#temporals` in the request locale (the platform zone, as every surface without a zone of its own) where it was `Instant.toString()`. **The guard nothing had**: `BundledTemplateTextTest`, a quote-aware tag scanner over the three apps — a text run with two letters outside an element whose body Thymeleaf replaces, a literal in a `th:text`/`th:replace`/… expression (the `shell('Portal', …)` title argument, `|${x} unread|`), a plain `placeholder`/`title`/`aria-label`/`alt`/`data-hc-confirm*` attribute; `<code>`/`<kbd>`/`<pre>` bodies exempt. 205 findings on HEAD, none on the fix. End to end: `DefaultLocalesIntegrationTest.theSignInPageRendersInTheNegotiatedLocale` (a zero-config app, `Accept-Language: ja` → `<h1>サインイン</h1>`) and `AccountSurfaceIntegrationTest.theAccountPageRendersInTheNegotiatedLocale` (`表示言語`, `セッション` under `lang="ja"`), both red on HEAD's templates. **8c SHIPPED — F127 CLOSED.** The 27 operator templates (`iam-admin` 16, `ops-console` 11): 446 new keys (`tql.iam.*` 274, `tql.ops.*` 172), English and Japanese. Done by a transformer, not by hand: a text-only parent gets `th:text`, a parent whose children are static inline markup (`<code>`, `<strong>`) gets `th:utext` with the inner HTML as the message, anything else wraps the run in a `<span th:text>`; plain `placeholder`/`title`/`aria-label`/`data-hc-confirm*` attributes become `th:<name>`; identical strings share a key within an app; the 27 page titles (`page('IAM Admin · Users', …)`, `ops-member-page(|Route Audit — ${x}|, …)`) and 13 `|…|` literals by hand as `#{key(${…})}`. **Three traps:** (1) an inline `[[${x}]]` expression is dynamic, not text — the transformer and the scanner strip it before judging (`[[${t.count}]] total` is the one run that carried a word beside one); (2) a `#{…}` cannot sit inside a `${…}` — OGNL reads it (`source is null for getProperty(null, \"iam\")`, a 500 on the user detail page, found by `IamAdminIntegrationTest`, 10 red) — `#messages.msg('key')` is the form; (3) a generated key can collide with a hand-added one (`activeSessions` the heading vs `{0} active sessions.`) and the merge overwrote the heading — `sessionsPageListsFiltersAndRevokesAcrossSubjects` caught it. A dotted identifier with no space (`.approver`, `tql.app.use.*`) is a name, not prose — the scanner exempts it. `BundledTemplateTextTest` now spans all five apps (the operator ones from the sibling modules by relative path, as `HcMarkupContractTest` does); 718 findings on HEAD, none on the fix. End to end: `IamAdminIntegrationTest.theConsoleRendersInTheNegotiatedLocale` (`IAM Admin · ユーザー`, `ユーザーを招待`) and `OpsConsoleIntegrationTest.theConsoleRendersInTheNegotiatedLocale` (`運用 — `, `バッチ実行`, `ヘルス`). Still English by design: the enum-valued option labels (`eq`, `in`, `subtree`, `network`, `hours`, `block`, `warn`) and status words the store returns (`UP`, `WARN`) — values, not prose. |
| 9 | The CLI refuses before it works | F114 | M | **SHIPPED** (decision 1 below, taken 2026-09-09). Measured first: `identity-schema --admin-login admin` is a stack trace and exit 1; with a database and no password it APPLIES THE SCHEMA, then throws. Ships: `UsageRefusal` (an `IllegalArgumentException` subclass — the handler shapes the subclass only, so a bug's IAE keeps its stack trace, guarded) thrown before any work from the connection set, `identity-schema` (password resolved before `applySchema`; an unreadable `--admin-password-file`), `token --ttl`, a job's undeclared datasource, `ModuleCoordinate.parse`, both `modules.lock` mismatches and `ModulesYaml`; `UnreachableDatabaseHandler` renamed `CliExceptionHandler` and mapping the refusal to one line + `ExitCode.USAGE` (2) on both binaries; the four codes declared once (`ExitCodes`) as picocli `exitCodeList` on both root commands, printed by `--help`, rendered by `CliReference` into `reference-cli.md`; `docs/cli-surface.md` decision 10 records the rule; the login page's hint gained `--app <dir>` (defect 6). Guards: `UsageRefusalTest` (5 of 6 red at HEAD on exit 1; the unreachable-database case pins 1), `AppLifecycleDbCommandsIntegrationTest` (PostgreSQL: the refusal leaves no `tql_users` — red on `v-after`, the HEAD order, on exactly that assertion), `CliExceptionHandlerTest` (the plain-IAE rethrow, red on `v-iae` — the audit's first suggestion), `TesseraqlHostCliTest` (the host declares the one list and refuses through the same shaper). **Not converted:** `EmbeddedPostgresBinary`/`EmbeddedPostgresSupport`'s `IllegalStateException`s (environment failures met while running — 1 is right), `CliModules:146` (unreachable), `McpDevTools:102` (`mcp` returns 2 before it). An H2 `MODE=PostgreSQL` cannot run the identity DDL, so the before-the-work guard had to be the PostgreSQL IT. |
| 10 | Sweeps | F82, F87, F88, F89, F90, F91, F96, F97, F98, F99, F112 | S each | **SHIPPED as one PR, five commits.** (a) F96+F97+F98: `--hc-border`/`--hc-color-accent`/`--hc-radius-md` → `--hc-color-border`/`--hc-color-focus-ring`/`--hc-control-radius` (the kit's generic control radius, 6px today, so nothing moves until a theme moves it); the two `:target` app rules deleted — **the adjudicators' dispute resolved by reading the shipped 0.4.0 CSS: the kit's dashed outline is inside `@media (forced-colors: active)` (line 13029 → 13092 of `hc.css`), so F97's adjudicator was right and nothing new appears in normal rendering**; the tint is the kit's accent-following `--hc-datagrid-highlight-bg` and the scroll margin its header-height calc. Guard `HypermediaComponentsManifestTest.everyKitTokenTheFrameworkStylesheetReadsIsOneTheKitDefinesOrReads` — "defined or read" over `hc.min.css` + `hc.tokens.css` with comments stripped, exactly the three phantoms on HEAD's stylesheet. `hypermedia-ui.md`'s stand-in sentence rewritten from `dist/confirm.js` (the kit submits the plain form itself, htmx-verb exemption included); `HcMarkupContractTest`'s Javadoc and violation text with it; `hc-recipe-alignment.md:264` now cites a page that describes the adopted state. (b) F88+F89+F90: `.dockerignore` names `vscode-extension/{node_modules,out,*.vsix}`, `.pnpm-store/`, `docs-site/.astro/` (guard `DockerignoreLedgerTest`: every entry's first segment exists or is gitignored — the two `editor/` entries on HEAD); `development-environment.md`'s wrapper section says the wrapper is committed; the PR template, CONTRIBUTING ("Before pushing") and `run-ci-local.sh` name the docs-site build and the extension test/package beside `mvn verify`. (c) F87+F91: `emailFragments.test.ts` moved to `test/` (73 extension tests run, up from 69; `src/test/` deleted); README, `docs/vscode-extension.md` and the status-bar tooltip say *Dev* / *Open Served App*. Guard `ExtensionLedgerTest`: every `*.test.ts` directly under `test/`, every advertised `*TesseraQL: X*` a manifest title — the misplaced file and the two stale names on HEAD. (d) F82+F112: README's Studio URL is the stack's, its recipe list is the surface's ten (three were missing) and links the generated reference; the Maven goal list was accurate. Guard `ReadmeLedgerTest` (recipes = the surface enum row, goals = the `@Mojo` names, no per-application Studio URL — two of three red on HEAD). Twenty-two Javadoc/comment `serve` sentences → `dev` (or `dev` and `host` where both carry the flag: `--env`, the module lock, the log flags); guard `RetiredVerbLedgerTest` over every main source, the one "replaces `serve`" sentence excepted. (e) F99: `StudioViews.PreviewChrome` — the shell's trio and sheets from the same UI beans `ShellChrome` reads, validated the same way, kit defaults publishing nothing; the page-builder canvas copies all four attributes. **Left, disclosed:** the signed-in user's stored theme is a per-request fact the `studio.render` provider has no request for — the preview renders the operator's default theme, and the ramp/density/accent exactly. F125's Location half was 4b; F82 slice 2 is its own campaign (decision 3). |
...
1. **The exit code of a CLI pre-flight refusal (slice 9).** Two conventions are live: 51
   ...
   unreachable-database shaping at 1. *Decided 2026-09-09: as recommended — a pre-flight refusal
   exits 2 (`docs/jobs.md:197` already publishes 2 for "a request that cannot run at all", so
   this widens a published contract rather than inventing one); the unreachable-database
   shaping stays 1. Slice 9 records the exit codes in `docs/cli-surface.md`.*
2. **Whether every app serves `ja` by default (slice 8).** The one-line candidate
   ...
   every application. That is a product decision, not a bug fix. *Decided 2026-09-09: include
   the built-in catalog's tags — the class's own Javadoc and `docs/internationalization.md:39`
   both already said "defaults to the catalogs found", so the code contradicted its
   documentation. Shipped as 8a, #1337.*
3. **Whether the second README quick start's framework defect is in scope.** F82's slice 2 is not a
   ...
   *Decided 2026-09-09: its own campaign.* The mechanism, so it is not re-filed a third time:
   ...
   its S1 closed the seam on 2026-09-14 and S4 the front door on 2026-09-15 — all four slices shipped.*
