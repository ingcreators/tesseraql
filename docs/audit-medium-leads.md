...
| F125 Content-Disposition has no RFC 6266 | **LIVE (widens)** | medium | filename half: PR 4a — 4a SHIPPED #nnnn; the Location half — a silent wrong page, and a post-sign-in return to the home page — is the more severe half and is PR 4b (`download-name-and-bytes.md`); the headline reachable path is the end user's own uploaded filename via the shipped `kind: attachment` |
