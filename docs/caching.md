...
> Closed 2026-09-22, #1428 and #1429: the request carries the topics and the tables as an
> import's does and the transfer service announces them where the extraction-timed statement
> commits; the transfer row records them at start (`V16__transfer_announcement.sql`) and the
> fetch that runs a download-timed one announces from the row — the operations console's fetch
> included; the two lints admit the recipe when it declares a follow-up and refuse the keys
> when it does not. The export rows of `ResultHoldIntegrationTest` and
> `LiveViewIntegrationTest` are the guards.
