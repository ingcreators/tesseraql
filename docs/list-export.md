...
> download-timed statement runs on the fetch, so #1428 had `FileDownloadProcessor` carry the
> route's declaration to the claim (the reviewed import's confirm-leg shape) and left the
> operations console's fetch — no route behind it — announcing nothing. **#1429** moved
> the declaration onto the transfer row (`V16__transfer_announcement.sql`: `emit_json`,
> `invalidates_json`, `emit_tenant_id`, recorded at start), so every first fetch announces
> from the row and the fetch-carried `Announcement` is gone; the console row of
> `ResultHoldIntegrationTest` is the guard. `TQL-YAML-1038` and `TQL-FIELD-4620` admit the
> recipe when it declares `export.after` and refuse the keys when it does not — an export with
> no follow-up writes nothing. Nothing of row 15 remains. One defect
...
| 5 | The grid page's regions (`tql/view/list.html`) | The search box sits **outside** the grid form (`:34-46`), `hx-get` on the page path with `hx-include` of the region's hidden inputs and no `hx-push-url`; the applied filters and `sort`/`dir` are hidden inputs **inside** the swapped region `#<id>-table` (`:106-116`); the status line, both result-cap surfaces and the pager are inside it too (`:117-174`); one `<form method="post">` wraps the region, the selection bar and the snapshot `keys` (`:72-82`), and its `_csrf` renders only when `actions:` or a snapshot is declared (`:73-75`); each bulk action is a submit button with its own `formaction` (`:99-101`). The filter dialog carries hidden `sort`/`dir`/search rendered at page render, outside the region (`:191-194`). **Since #1430 the search box replaces the URL, the sort headers push it, and the dialog's copies and the chips are refreshed out of band.** |
...
- **A job inbox** ("my exports"). The upstream note — the recipe applied per row over a
  ...
  **Designed and shipped 2026-09-22: [job-inbox.md](job-inbox.md)** — the column first
  (#1432), then a bundled "My exports" page mounted with the application (#1433), then the
  grid page's region remembering (#1434).
...
- **A `file-export`'s `after:` commit announces nothing** (row 15) — **closed, #1428 and
  #1429** (the status block): `announcing`/`invalidating` on `ExportRequest` for the
  extraction-timed leg, the declaration recorded on the transfer row at start for the
  download-timed one, the lints admitting the recipe with a follow-up. #1428 had left one fetch
  unannounced — the operations console's, which knows no route — and #1429 closed it by
  moving the declaration from the fetch to the row, which every first fetch reads.
- **The in-place search does not push the URL** (`list.html:34-41` had no `hx-push-url`), so the
  address bar, the filter dialog's hidden search (`:193-194`) and a bookmark fell behind a typed
  search until the next navigation; the export control is inside the region and was not
  affected, the chips and the dialog were — **closed, #1430**: the search box replaces the
  URL, the sort headers push it, and the dialog's carried state and the condition bar are
  refreshed out of band by every in-place swap (`hx-select-oob` on the section; the
  list-surface record's status block carries the note). Nothing filed by this record stays
  open; the three entries below are recorded so they are not reported as defects.
...
- **A second kick-off hides the first card** (decision 5). The run continues and its status
  ...
  answer, when triggered — **closed, #1434** ([job-inbox.md](job-inbox.md) decision 8), and
  the snapshot pager's (decision 5) with it, on the grid page itself: the region is filled
