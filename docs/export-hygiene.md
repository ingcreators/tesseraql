...
| 3 | `FileCodecs.discover` last-put-wins | LIVE, wider — a module codec answering `csv` decides the async export, the job step AND every import arm; never the sync route (F82's class-loader split). No log line in any sink. | `TesseraqlRuntime:1045`, `FileCodecs.discover` | P7 (one WARN); the lint and the boot refusal shipped as [`codec-discovery.md`](codec-discovery.md) S2, after S1 gave the sync route the same set |
