...
bundled system apps: the account, inbox, task-queue, sign-in, invitation, password-recovery,
consent and portal pages, and the IAM Admin and Operations consoles), so any framework
message can be overridden per app. Texts may carry `{name}`
