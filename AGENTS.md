...
TesseraQL is a SQL-first hypermedia and integration framework that compiles simple external YAML into its own compiled pipelines, executes SQL through TesseraQL's own pipeline steps, renders JSON/HTML responses, and provides tests, coverage, operations, and Studio tooling.

## Current baseline

- Java baseline: 25
- Build tool: Maven
- Runtime direction: the framework's own pipeline runtime, no integration engine
...
2. Keep `tesseraql-core` free from pipeline, runtime, and CLI dependencies unless explicitly intended.
3. Do not introduce runtime dependencies in core modules without explaining the module boundary.
4. Keep user-facing YAML simple; avoid leaking framework internals into standard examples.
