...
public final class TesseraqlRuntime implements AutoCloseable {
     ...
     */
    private static io.vertx.core.VertxOptions vertxOptions(AppConfig config) {
    ...
    private static TesseraqlRuntime start(Path appHome, AppManifest loaded, int port,
        ...
        // The Vert.x instance the HTTP server builds is sized here rather than inherited
        // (docs/http-threading.md decision 1). Camel resolves VertxOptions by type from this
        // registry, so binding one is the whole mechanism.
