 ...
 * @param coverableLines source lines the template could emit (the line denominator)
 ...
 * @param lineRatio      {@code coveredLines / coverableLines}; 1.0 when the denominator is unknown
 */
public record SqlCoverageReport(
        String sqlId,
        Set<Integer> coveredLines,
        Set<Integer> coverableLines,
        int branchCount,
        int branchOutcomes,
        double branchRatio,
        double lineRatio) {
    ...
    public int coverableLineCount() {
        return coverableLines.size();
    }
}
