...
class AppTestRunnerIntegrationTest {
    ...
    void runsExampleSuitesWritesReportsAndCollectsCoverage(@TempDir Path reportDir) {
        ...
        assertThat(result.kind("http").declared()).containsExactly("directory.sync.headcount");
        assertThat(result.kind("http").covered()).containsExactly("directory.sync.headcount");
        assertThat(result.kind("http").ratio()).isEqualTo(1.0);
