...
public final class CrudScaffolder {
        ...
        return """
                ...
                response:
                  html:
                    ...
                    model:
                      rows: sql.rows
                %s""".formatted(names.table(), names.entity(), names.pkField(), names.pkField(),
                CSP_HEADERS);
    }
    ...
    private static String updateRoute(TableSchema table, Names names) {
        ...
        route.append("""
                ...
                names.entity()));
        List<TableSchema.Column> inputs = new ArrayList<>(formColumns(table, names));
        ...
        route.append("  params:\n    ").append(names.pkField()).append(": path.")
                .append(names.pkField()).append('\n');
        for (TableSchema.Column column : formColumns(table, names)) {
            route.append("    ").append(Names.field(column)).append(": body.")
                    .append(Names.field(column)).append('\n');
        }
        if (locked) {
            route.append("    version: body.version\n");
        }
        route.append("""
    ...
    private static String paramsBlock(String indent, List<TableSchema.Column> columns) {
        ...
        for (TableSchema.Column column : columns) {
            params.append(indent).append("  ").append(Names.field(column)).append(": body.")
                    .append(Names.field(column)).append('\n');
        }
