...
class HtmxContractGeneratorTest {
    ...
    void listsPagesAndFragmentsWithTemplatesInputsAndSecurity() {
        ...
        assertThat(pages).anySatisfy(page ->
                assertThat(page.get("path")).isEqualTo("/users"));
        assertThat(fragments).anySatisfy(fragment -> {
            ...
            assertThat(security).containsEntry("auth", "browser").containsEntry("policy", "users.read");
        });

        // JSON API routes are not part of the hypermedia contract.
        assertThat(pages).noneSatisfy(page ->
                assertThat(String.valueOf(page.get("path"))).startsWith("/api/"));
        assertThat(fragments).noneSatisfy(fragment ->
                assertThat(String.valueOf(fragment.get("path"))).startsWith("/api/"));
    }
