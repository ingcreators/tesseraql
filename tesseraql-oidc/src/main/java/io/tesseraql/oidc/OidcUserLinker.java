...
/**
 * Links a federated OIDC identity to a local identity-store user (design ch. 10.14 userLink): it
 ...
 * unknown federated subject signs in (just-in-time provisioning). Mirrors the SAML user linker.
 */
final class OidcUserLinker {
    ...
    Principal resolve(String loginId, String displayName, String email, String tenantId) {
        ...
        if (!provision) {
            throw new OidcException("No local account is linked to '" + loginId + "'");
        }
        ...
        return identity.resolvePrincipal(realm, loginId, tenantId)
                .orElseThrow(() -> new OidcException("Provisioned user not found: " + loginId));
    }
}
