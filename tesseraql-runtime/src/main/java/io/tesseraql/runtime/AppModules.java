...
final class AppModules implements AutoCloseable {
    ...
    /**
     * Loads the application's modules — from the directory {@code WorkHome.moduleSet} names:
     * the set its package carries ({@code .tesseraql/modules}) when it has one, otherwise
     * {@code work/modules} under its work home — composed with {@code extraModules} when the
     * development override is set. Neither present → no child loader, built-ins only, nothing
     * to close.
     *
     ...
     * reads {@code work/modules} as it always did — and so does every CLI verb, through the same
     * function (docs/codec-discovery.md decision 4).
     */
    static AppModules load(Path appHome, io.tesseraql.yaml.config.AppConfig config,
            File extraModules) {
        List<File> dirs = new ArrayList<>();
        dirs.add(io.tesseraql.yaml.config.WorkHome.moduleSet(appHome, config).toFile());
